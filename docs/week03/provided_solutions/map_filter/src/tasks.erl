...
%%% @end
%%%-------------------------------------------------------------------
c_map(List,_Predicate) when is_list(List) == false ->
	no_list_error;
...
%%% @end
%%%-------------------------------------------------------------------
c_filter(List,_Predicate) when is_list(List) == false ->
	no_list_error;
...
pick_candidates(Applicants)->
	Beginners = c_filter(Applicants,fun({_name,Experience,Skills}) -> 
										(Experience =< 2) and 
										(lists:member(erlang,Skills) or 
										 lists:member(javascript,Skills))
									end),
	c_map(Beginners,fun({Name,Experience,Skills}) -> 
							case lists:member(erlang,Skills) of
								true -> {Name,Experience,backend,Skills};
								_ -> {Name,Experience,frontend,Skills}
