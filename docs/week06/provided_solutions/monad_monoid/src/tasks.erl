...
merge_helper([],List_b)->List_b;
merge_helper(List_a,[])->List_a;
merge_helper([Ha|Ta],[Hb|Tb])->
	[Ha,Hb]++merge_helper(Ta,Tb).
...
monad_chain(Monadal_type,Binding_lambda,Data_manipulators)->
	lists:foldl(fun(Data_manipulator,Accum) -> Binding_lambda(Accum,Data_manipulator) end,
		        Monadal_type,
		        Data_manipulators).
