...
speed_test() ->
	...
	Start_standard = os:timestamp(),
  	Standard_text = lists:foldl(fun(C,Accume)-> C++" "++Accume end,"",TLAs),
  	io:format("standard foldl function total time taken ~f seconds~n~n", [timer:now_diff(os:timestamp(), Start_standard) / 1_000_000]),

	Start_custom = os:timestamp(),
  	Custom_text = c_foldl(TLAs,"",fun(Accume,C)-> C++" "++Accume end),
  	io:format("custom foldl function total time taken ~f seconds~n~n", [timer:now_diff(os:timestamp(), Start_custom) / 1_000_000]),
